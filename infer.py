...
if __name__ == '__main__':
    ...
    pth = 'checkpoints/fuji/nn/best.pth'
    model.load_state_dict(torch.load(pth, map_location=device))
    model.to(device)
    model.eval()
    st = time.time()
    target = infer(image='/Users/maoyufeng/Downloads/100_FUJI/jpg/nn/DSCF1861_org.jpg', model=model)
    print(time.time()-st)
    # print(target.size)
    target.show()
    # target.save('/Users/maoyufeng/Downloads/dog.jpg')

...
class Trainer:
    def __init__(self, model, data_path, save_model_path,channel='rgb',resize=(448,448), pretrained_model_path=None, test_image=None):
        """
        model: 模型
        data_path: 训练数据
    ...
    def train(self, epoch_num=200, lr=0.00001, batch_size=8, eval_step=5, early_stop_step=10, save_cfg=True):
        ...
        """
        train_data = MaskDataset(dataset_path=os.path.join(self.data_path), mode='train', channel=self.channel,resize=self.train_image_size)
        val_data = MaskDataset(dataset_path=os.path.join(self.data_path), mode='val', channel=self.channel,resize=self.train_image_size)
...
if __name__ == '__main__':
    trainer = Trainer(data_path='/Users/maoyufeng/slash/dataset/train_dataset/pro400h',
                      model=FilterSimulation(),
                      save_model_path='static/checkpoints/fuji/pro400h',
                      pretrained_model_path=None,
                      channel='rgb')
    trainer.train(epoch_num=200, lr=1e-4, batch_size=8, eval_step=1, early_stop_step=20)
